...
CREATE TABLE academic_term_status (
    term_status_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    code VARCHAR(50) NOT NULL UNIQUE,
    name VARCHAR(100) NOT NULL,
    active BOOLEAN NOT NULL DEFAULT TRUE,
    sort_order INT NOT NULL,
...
CREATE TABLE course_offering_status (
    course_offering_status_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    code VARCHAR(50) NOT NULL UNIQUE,
    name VARCHAR(100) NOT NULL,
    active BOOLEAN NOT NULL DEFAULT TRUE,
    sort_order INT NOT NULL,
...
CREATE TABLE academic_term (
   ...
   CONSTRAINT uq_academic_term_id_year
       UNIQUE (term_id, academic_year_id),

   CONSTRAINT chk_academic_term_date_range
       CHECK (start_date <= end_date)
);

CREATE TABLE academic_term_group (
    term_group_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    CONSTRAINT fk_academic_term_group_academic_year
        FOREIGN KEY (academic_year_id) REFERENCES academic_year(academic_year_id),

    CONSTRAINT uq_academic_term_group_year_code
        UNIQUE (academic_year_id, code),

    CONSTRAINT chk_academic_term_group_date_range
        CHECK (start_date <= end_date)
);

CREATE TABLE academic_term_group_term (
    term_group_id BIGINT NOT NULL,
    term_id BIGINT NOT NULL,

    PRIMARY KEY (term_group_id, term_id),

    CONSTRAINT uq_academic_term_group_term_term
        UNIQUE (term_id),

    CONSTRAINT fk_academic_term_group_term_group
        FOREIGN KEY (term_group_id) REFERENCES academic_term_group(term_group_id),

    CONSTRAINT fk_academic_term_group_term_term
...
CREATE TABLE course_offering_term (
    course_offering_id BIGINT NOT NULL,
    term_id BIGINT NOT NULL,
    academic_year_id BIGINT NOT NULL,

    PRIMARY KEY (course_offering_id, term_id),

    CONSTRAINT fk_course_offering_term_offering
        FOREIGN KEY (course_offering_id, academic_year_id)
            REFERENCES course_offering(course_offering_id, academic_year_id),

    CONSTRAINT fk_course_offering_term_term
        FOREIGN KEY (term_id, academic_year_id)
            REFERENCES academic_term(term_id, academic_year_id)
);
...
CREATE TRIGGER trg_academic_term_last_updated
BEFORE UPDATE ON academic_term
FOR EACH ROW
EXECUTE FUNCTION set_last_updated_timestamp();
